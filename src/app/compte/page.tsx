  ...
  return (
    <div style={{ maxWidth: 900, margin: "0 auto", padding: "0 40px" }}>
      <div style={{ padding: "18px 0 14px" }}>
        ...
        <h1 style={{ fontSize: 28, fontWeight: 800, color: C.text, marginTop: 6 }}>Mon compte 🍿</h1>
      </div>
      <div style={{ display: "flex", gap: 4, marginBottom: 18, padding: 4, background: C.bgAlt, borderRadius: 12, width: "fit-content", flexWrap: "wrap" }}>
        <button onClick={() => { setTab("inscriptions"); setSearch("") }} style={{ padding: "7px 14px", borderRadius: 9, border: "none", background: tab === "inscriptions" ? C.accentBg : "transparent", color: tab === "inscriptions" ? C.accent : C.textTer, fontSize: 12, fontWeight: tab === "inscriptions" ? 700 : 500, cursor: "pointer" }}>📋 Inscriptions ({inscF.length})</button>
        <button onClick={() => { setTab("done"); setSearch("") }} style={{ padding: "7px 14px", borderRadius: 9, border: "none", background: tab === "done" ? C.yellowBg : "transparent", color: tab === "done" ? C.yellowDark : C.textTer, fontSize: 12, fontWeight: tab === "done" ? 700 : 500, cursor: "pointer" }}>✓ Mes formations ({doneF.length})</button>
        <button onClick={() => setTab("avis")} style={{ padding: "7px 14px", borderRadius: 9, border: "none", background: tab === "avis" ? "rgba(232,123,53,0.1)" : "transparent", color: tab === "avis" ? "#E87B35" : C.textTer, fontSize: 12, fontWeight: tab === "avis" ? 700 : 500, cursor: "pointer" }}>⭐ Mes avis ({myAvis.length})</button>
        <button onClick={() => setTab("favoris")} style={{ padding: "7px 14px", borderRadius: 9, border: "none", background: tab === "favoris" ? C.pinkBg : "transparent", color: tab === "favoris" ? C.pink : C.textTer, fontSize: 12, fontWeight: tab === "favoris" ? 700 : 500, cursor: "pointer" }}>❤️ Favoris</button>
      </div>
        ...
        <div>
          {myAvis.length === 0 ? <div style={{ textAlign: "center", padding: 40, color: C.textTer }}>Vous n&apos;avez pas encore laissé d&apos;avis.</div> :
            <div style={{ display: "flex", flexDirection: "column", gap: 10, paddingBottom: 40 }}>
              {myAvis.map(a => {
                const formation = FORMATIONS.find(ff => ff.id === a.formationId);
                return (
                  <Link key={a.id} href={`/formation/${a.formationId}`} style={{ textDecoration: "none", color: "inherit" }}>
                    <div style={{ padding: 16, background: C.surface, borderRadius: 14, border: "1px solid " + C.borderLight, cursor: "pointer" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                        <span style={{ fontSize: 14, fontWeight: 700, color: C.text }}>{formation?.titre || "Formation"}</span>
      ...
      {(tab === "inscriptions" || tab === "done") && (
        <div>
          ...
          {tab === "inscriptions" && inscF.length > 0 && (
            <div style={{ marginBottom: 20, padding: 16, background: C.surface, borderRadius: 14, border: "1px solid " + C.borderLight }}>
              ...
              {inscF.flatMap(f => f.sessions.map((s, i) => ({ ...s, titre: f.titre, fId: f.id, key: f.id + "-" + i }))).sort((a, b) => a.dates.localeCompare(b.dates)).map(s => (
                <Link key={s.key} href={`/formation/${s.fId}`} style={{ textDecoration: "none", color: "inherit" }}>
                  <div style={{ display: "flex", gap: 10, alignItems: "center", padding: "8px 12px", background: C.gradientBg, borderRadius: 9, cursor: "pointer", border: "1px solid " + C.borderLight, marginBottom: 4 }}>
          ...
          {filtered.length === 0 ? <div style={{ textAlign: "center", padding: 40, color: C.textTer }}>Aucune formation ici.</div> :
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(300px,1fr))", gap: 10, paddingBottom: 40 }}>
