                  ...
                  {sessions.map((s, i) => (
                    <div key={i} style={{ padding: 16, background: C.surface, borderRadius: 14, border: "1.5px solid " + C.border, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: 13, color: C.textSec }}>📍 {s.lieu}</div>
                        {s.adresse && <div style={{ fontSize: 12, color: C.textTer, marginTop: 2 }}>{s.adresse}</div>}
                        {(s as any).parties && (s as any).parties.length > 0 && (
                          <div style={{ marginTop: 8, paddingTop: 8, borderTop: "1px dashed #E8D5A3" }}>
                            {(s as any).parties.map((p: any, pi: number) => (
                              <div key={pi} style={{ display: "flex", gap: 8, alignItems: "center", fontSize: 12, color: C.textSec, marginBottom: 3 }}>
                                <span style={{ fontWeight: 600, color: C.accent }}>Partie {pi+1}</span>
                                {p.titre && <span>{p.titre}</span>}
                                <span style={{ padding: "1px 6px", borderRadius: 4, background: p.modalite === "Visio" ? C.accentBg : C.greenBg, color: p.modalite === "Visio" ? C.accent : C.green, fontSize: 10, fontWeight: 700 }}>{p.modalite}</span>
                      ...
                      <button onClick={() => handleInscription(s.id)} disabled={inscribing} style={{ padding: "10px 20px", borderRadius: 10, background: C.gradient, color: "#fff", fontSize: 13, fontWeight: 700, border: "none", cursor: "pointer", opacity: inscribing ? 0.7 : 1 }}>
                        {inscriptions.some(i => i.formation_id === f.id && i.session_id === s.id) ? "✓ Inscrit·e" : "S'inscrire"}
                      </button>
                    </div>
                  ))}
                </div>
