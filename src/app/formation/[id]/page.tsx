...
function AvisSection({ formationId, avis, onAdd, onEdit, mob }: { formationId: number; avis: Avis[]; onAdd: (fId: number, note: number, texte: string) => void; onEdit: (aId: number, note: number, texte: string) => void; mob: boolean }) {
  ...
  return (
    <div style={{ marginBottom: 24 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
        ...
        {!myAvis && !showForm && <button onClick={() => { setShowForm(true); setEditMode(false); setTexte(""); setNote(5) }} style={{ padding: "7px 14px", borderRadius: 9, border: "none", background: C.gradient, color: "#fff", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>Donner mon avis</button>}
      </div>
      {showForm && (
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {fAvis.map(a => (
          <div key={a.id} style={{ padding: mob ? 10 : 14, background: a.userId === "me" ? C.yellowBg : C.surface, borderRadius: 12, border: "1px solid " + (a.userId === "me" ? C.yellow + "33" : C.borderLight) }}>
...
export default function FormationPage() {
  ...
  const addAvis = (fId: number, n: number, t: string) => { setAvis(p => [...p, { id: Date.now(), formationId: fId, userId: "me", userName: "Vous", note: n, texte: t, date: new Date().toISOString().slice(0, 10) }]) };
  const editAvis = (aId: number, n: number, t: string) => setAvis(p => p.map(a => a.id === aId ? { ...a, note: n, texte: t } : a));
  const px = mob ? "0 16px" : "0 24px";
  ...
  return (
    <div style={{ maxWidth: 920, margin: "0 auto", padding: px }}>
      ...
      <div style={{ borderRadius: mob ? 14 : 22, overflow: "hidden", marginBottom: mob ? 14 : 24, position: "relative" }}>
        ...
        <div style={{ position: "absolute", bottom: mob ? 14 : 28, left: mob ? 14 : 28, right: mob ? 14 : 28 }}>
          ...
          {f.sousTitre && <p style={{ fontSize: mob ? 12 : 14, color: "rgba(255,255,255,0.7)", marginTop: 4, fontStyle: "italic" }}>{f.sousTitre}</p>}
        </div>
      </div>
      ...
      <div style={{ display: mob ? "flex" : "grid", gridTemplateColumns: "1fr 280px", flexDirection: "column", gap: mob ? 14 : 24, paddingBottom: 30 }}>
        ...
        <div>
          ...
          {f.motsCles && <div style={{ marginBottom: 20 }}>{lbl("Mots-clés")}<div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>{f.motsCles.map(m => <span key={m} style={{ padding: "4px 10px", borderRadius: 8, fontSize: 11, background: C.yellowBg, color: C.yellowDark, fontWeight: 600 }}>{m}</span>)}</div></div>}
          {f.populations && <div style={{ marginBottom: 20 }}>{lbl("Population")}<div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>{f.populations.map(p => <span key={p} style={{ padding: "4px 10px", borderRadius: 8, fontSize: 11, background: C.blueBg, color: C.blue, fontWeight: 600 }}>{p}</span>)}</div></div>}
          {f.professions && <div style={{ marginBottom: 20 }}>{lbl("Professions")}<div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>{f.professions.map(p => <span key={p} style={{ padding: "4px 10px", borderRadius: 8, fontSize: 11, background: C.greenBg, color: C.green, fontWeight: 600 }}>{p}</span>)}</div></div>}
          ...
          {fmt && <div style={{ marginBottom: 20 }}>{lbl(fmtTitle(f.formateurId))}
          ...
          {lbl("Critiques")}
          <AvisSection formationId={f.id} avis={avis} onAdd={addAvis} onEdit={editAvis} mob={mob} />
          {otherFmt.length > 0 && <div style={{ marginBottom: 24 }}>{lbl(fmtLabel(f.formateurId))}<div style={{ display: "grid", gridTemplateColumns: mob ? "1fr" : "1fr 1fr", gap: 10 }}>{otherFmt.slice(0, 4).map(x => <FormationCard key={x.id} f={x} compact mob={mob} />)}</div></div>}
          {otherOrg.length > 0 && <div style={{ marginBottom: 24 }}>{lbl("D'autres formations de " + (org?.nom || ""))}<div style={{ display: "grid", gridTemplateColumns: mob ? "1fr" : "1fr 1fr", gap: 10 }}>{otherOrg.slice(0, 4).map(x => <FormationCard key={x.id} f={x} compact mob={mob} />)}</div></div>}
          {otherLike.length > 0 && <div style={{ marginBottom: 24 }}>{lbl("D'autres formations qui pourraient vous plaire")}<div style={{ display: "grid", gridTemplateColumns: mob ? "1fr" : "1fr 1fr", gap: 10 }}>{otherLike.map(x => <FormationCard key={x.id} f={x} compact mob={mob} />)}</div></div>}
        </div>
