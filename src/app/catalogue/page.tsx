...
function CatalogueContent() {
  ...
  let filtered = FORMATIONS.filter(f => {
    if (search) { const q = search.toLowerCase(); if (![f.titre, f.sousTitre || "", f.domaine, ...(f.motsCles || []), ...(f.populations || []), ...f.sessions.map(s => s.lieu)].some(s => s.toLowerCase().includes(q))) return false }
  ...
  return (
    <div style={{ maxWidth: 1240, margin: "0 auto", padding: px }}>
      <div style={{ padding: "18px 0 14px" }}>
        ...
        <h1 style={{ fontSize: mob ? 22 : 28, fontWeight: 800, color: C.text, marginTop: 6 }}>{ville ? `Formations à ${ville}` : "Toutes les formations"}</h1>
        <p style={{ fontSize: mob ? 12 : 13, color: C.textTer, marginTop: 3 }}>{filtered.length} formation{filtered.length > 1 ? "s" : ""}</p>
      </div>
      <div style={{ display: "flex", gap: mob ? 6 : 10, marginBottom: mob ? 12 : 20, flexWrap: mob ? "wrap" : "nowrap" }}>
        <div style={{ flex: 1, minWidth: mob ? "100%" : "auto", display: "flex", alignItems: "center", gap: 8, padding: "0 12px", background: C.surface, border: "1.5px solid " + C.border, borderRadius: 12, height: mob ? 38 : 44 }}>
