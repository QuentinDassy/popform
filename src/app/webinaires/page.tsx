...
export default function WebinairesPage() {
  ...
  return (
    <div style={{ maxWidth: 1100, margin: "0 auto", padding: mob ? "0 16px" : "0 40px" }}>
      ...
      {loading ? (
        <div style={{ textAlign: "center", padding: 60, color: C.textTer }}>🍿 Chargement…</div>
      ) : displayed.length === 0 ? (
        ...
        <div style={{ display: "grid", gridTemplateColumns: mob ? "1fr" : "repeat(auto-fill, minmax(340px, 1fr))", gap: mob ? 14 : 20, paddingBottom: 48 }}>
          {displayed.map(w => {
            ...
            const past = isWebPast(w);
            return (
              <div key={w.id} style={{ background: C.surface, border: "1.5px solid " + (live ? "#DC262633" : C.borderLight), borderRadius: 18, overflow: "hidden", display: "flex", flexDirection: "column", opacity: past ? 0.75 : 1 }}>
                <div style={{ padding: mob ? "18px 16px 14px" : "22px 22px 16px", flex: 1, display: "flex", flexDirection: "column", gap: 10 }}>
                  ...
                  <div style={{ display: "flex", gap: 7, flexWrap: "wrap" }}>
                    ...
                    {past && <span style={{ fontSize: 11, fontWeight: 700, padding: "3px 10px", borderRadius: 8, background: C.bgAlt, color: C.textTer }}>Passé</span>}
                  </div>
